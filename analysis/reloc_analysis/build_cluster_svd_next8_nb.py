#!/usr/bin/env python
"""Generate 34.UF_cluster_svd_next8.ipynb — per-volume LSQR-CND relocation + global-bootstrap uncertainties for
the NEXT 8 largest-event clusters (ranks 3-10 by mainshock magnitude, the nb31 set), the companion to nb33
(which did the top-2, M3.89 & M3.73). Same methodology and house style as nb33.

Reported solution = LSQR-CND seeded on the whole-box LSQR locations (light damping, CND 40-80). Absolute centroid
depth is a DD near-null-space direction — undamped SVD wanders down it (e.g. c4: 11.4 -> 7.1 km), LSQR-CND holds
the physical whole-box centroid and gives the same relative structure. Bootstrap = global resampling, LSQR-CND,
n=200; ez95 = RELATIVE precision (absolute centroid ~km, uncaptured).

Reads ONLY cached runner outputs (uf_subregion_hypodd/svd_volumes_next8/c{11,4,40,41,1,105,8,95}/): hypoDD.reloc
(LSQR-CND), hypoDD.reloc.svd (SVD diagnostic), bootstrap/*, analysis/*, volume_events.csv, run_meta.json. Base env.
...
md(r"""# Per-volume relocation of the next 8 largest-event clusters (ranks 3–10) — companion to nb33

The same per-volume analysis nb33 applied to the two largest-event volumes (M3.89 2014, M3.73 2023), extended to
the **next 8 largest-earthquake clusters** (ranks 3–10 by mainshock magnitude, M2.00–2.67 — the nb31 set):
the 8 clusters ranked 3–10 by mainshock magnitude (M2.0–2.7; the current NND labels are listed by the load cell
below and tracked via `volumes.txt`, since they change when the catalog is re-relocated). For each, the full 1 km³ in-cube population (all relocated events, incl. ML-less)
is re-relocated stand-alone.
...
BASE="/home/msseo/works/02.Ulsan_Fault_detection/analysis/uf_subregion_hypodd/svd_volumes_next8"
...
_vtxt="/home/msseo/works/02.Ulsan_Fault_detection/analysis/uf_subregion_hypodd/svd_volumes_next8/volumes.txt"
...
for v in VOLS:
    ...
    TITLE[v]=f"{v} — M{meta['mainshock_mag']:.2f} ({str(meta['mainshock_time'])[:10]})"
...
# ------------------------------------------------------------------ §5 summary
md(r"""## 5 · Summary""")
co(r"""print("="*140); print("PER-VOLUME LSQR-CND RELOCATION (whole-box seed) + GLOBAL BOOTSTRAP — next 8 largest-event UF clusters (ranks 3-10)".center(140)); print("="*140)
print(SUM.to_string(index=False))
print("\nTAKE-HOMES")
...
nbf.write(nb,"34.UF_cluster_svd_next8.ipynb")
print("wrote 34.UF_cluster_svd_next8.ipynb",len(C),"cells")
