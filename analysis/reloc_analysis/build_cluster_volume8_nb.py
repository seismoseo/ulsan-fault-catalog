#!/usr/bin/env python
"""Generate 31.UF_cluster_volume_history_next8.ipynb — volume history of the NEXT 8 LARGEST-EVENT clusters
(ranks 3-10 by mainshock magnitude; companion to nb30 which did the top 2, M3.89 & M3.73). For each: center a 1 km^3 cube on the cluster centroid, pull EVERY
...
md(r"""# Volume history of the next 8 largest-event clusters (ranks 3–10) — companion to nb30

nb30 examines the two largest-earthquake volumes (M3.89 2014, M3.73 2023). Here we apply the **identical
1 km³ cube analysis** to the **next 8 largest-earthquake clusters** (ranks 3–10 by mainshock magnitude) to ask
whether their interseismic behaviour matches. NND links every event to its nearest neighbour in **rescaled time** τ, so it
...
TARGETS=list(fammax.iloc[2:10].index)                    # ranks 3-10 by mainshock magnitude (nb30 did ranks 1-2)
print("next 8 largest-event clusters (reliable NND, 1 km cap):")
for k in TARGETS:
...
allc["suspect"]=(allc.shift_km>3.0)|(allc.nlinks<10)|(allc.ncc==0)  # weak/large-move OR ct-only (cc-starved)
...
`suspect` (moved >3 km in relocation, or <10 used links) are counted separately — their in-cube presence may be
spurious.""")
co(r"""HW=HALF*1000.0
def build_box(k):
    ...
    box=allc[in_cube|allc.event_idx.isin(memset)].copy()   # UNION cube with full NND family: no member omitted
...
_cc=plt.cm.tab10(np.linspace(0,1,10))
...
ax.set(xlim=UF[:2],ylim=UF[2:],xlabel="Longitude (°E)",ylabel="Latitude (°N)",title="Next 8 largest-event clusters (ranks 3–10) — 1 km³ cube footprints")
ax.set_aspect(1/np.cos(np.deg2rad(35.75))); ax.legend(loc="upper left",fontsize=7.5,ncol=2); plt.show()""")
...
print("="*112); print(f"1 km³ VOLUME HISTORY — next 8 largest-event UF clusters, decomposed (ranks 3–10, half-width {HALF} km)".center(112)); print("="*112)
...
nbf.write(nb,"31.UF_cluster_volume_history_next8.ipynb")
print("wrote 31.UF_cluster_volume_history_next8.ipynb",len(C),"cells")
