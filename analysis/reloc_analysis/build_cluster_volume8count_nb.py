#!/usr/bin/env python
"""Generate 32.UF_cluster_volume_history_top8count.ipynb — volume history of the 8 largest clusters by MEMBER
COUNT (Cluster 0-7 under descending-size labeling; companion to nb30/nb31 which ranked by mainshock magnitude). For each: center a 1 km^3 cube on the cluster centroid, pull EVERY
...
md(r"""# Volume history of the 8 largest clusters (by member count) — was the volume active before?

The 8 **richest NND families** (largest member counts; Cluster 0–7) are the most developed sequences on the
fault. nb30/nb31 ranked by mainshock magnitude; here we rank by **member count**. Same 1 km³ cube method. NND links every event to its nearest neighbour in **rescaled time** τ, so it
...
TARGETS=list(famsize.head(8).index)                      # 8 largest families by MEMBER COUNT
print("8 largest clusters by member count (reliable NND, 1 km cap):")
for k in TARGETS:
...
ax.set(xlim=UF[:2],ylim=UF[2:],xlabel="Longitude (°E)",ylabel="Latitude (°N)",title="8 largest clusters by member count — 1 km³ cube footprints")
...
print("="*112); print(f"1 km³ VOLUME HISTORY — 8 largest clusters by member count, decomposed (Cluster 0–7, half-width {HALF} km)".center(112)); print("="*112)
...
nbf.write(nb,"32.UF_cluster_volume_history_top8count.ipynb")
print("wrote 32.UF_cluster_volume_history_top8count.ipynb",len(C),"cells")
