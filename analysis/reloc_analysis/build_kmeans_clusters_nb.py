#!/usr/bin/env python
"""Generate 36.UF_kmeans_spatial_clusters.ipynb — K-means (K=7) spatial clustering of the kim2011 whole-box
...
md(r"""# K-means spatial clusters of the UF catalog — steadiness & fractal dimension

The kim2011 whole-box dt.cc HypoDD relocation (ISTART=2 adaptive) is partitioned into **7 spatial clusters** with
K-means, so each cluster is a *map-identifiable patch* of the Ulsan Fault. This is a deliberately simpler,
complementary view to the NND (Zaliapin–Ben-Zion) families of nb28–nb35: NND groups by *rescaled-time* nearest
neighbour (a declustering statistic); K-means groups by *geometry*, giving the handful of blobs a reader actually
sees on the map. Every relocated event is assigned to exactly one cluster. **The 56 quarry-blast events (two
daytime pits, identified in nb22 §7) are removed first** — otherwise they form their own spurious shallow K-means
clusters; this is the **de-blasted** catalog.
...
print(f"loaded {len(r0)} relocated events | {int(r0.has_ml.sum())} with ML | {int(r0.suspect.sum())} suspect | "
...
DENS=dgrid(r0.lon.values,r0.lat.values)                       # smoothed count density of ALL relocated events
...
print("="*140); print("K-MEANS SPATIAL CLUSTERS (K=7) — STEADINESS & FRACTAL DIMENSION — kim2011 relocated UF catalog".center(140)); print("="*140)
...
nbf.write(nb,"36.UF_kmeans_spatial_clusters.ipynb")
print("wrote 36.UF_kmeans_spatial_clusters.ipynb",len(C),"cells")
