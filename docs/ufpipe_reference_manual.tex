...
{\small\color{soft} Long-term (2010–present) catalog for the Ulsan Fault, SE Korea\hfill rev.\ 2026-08-04 · \code{src/ufpipe}}
\end{center}
...
\begin{warnbox}{NS station epochs --- moved stations are SPLIT and association is the rename boundary}
22 of the 220 GHBSN (NS) stations were physically relocated during operation (12\,m--5.2\,km; N108 and N166
twice). Each position epoch is its OWN station: lowercase suffixes from the \emph{first} epoch
(\code{N010a}, \code{N010b}; a plain code always means never-moved). The canonical epoch table
(\code{ufpipe.ns\_epochs}, generated from the user-validated SOTA list
\code{stations/ns/20240715/GHBSN\_info\_ver202312\_modified.csv}) is the single source; the old list carried a
13\,km paste error at N019 and the epochs were previously AVERAGED into fictitious midpoints. Detection and the
raw picks always use BASE codes (position-free; the archive stores base-code dirs); at the association input,
each NS pick is stamped with the epoch code in force at its pick time, and every downstream product ---
assignment, \code{.phs}, \code{.sta}, \code{station.dat}, SAC filenames and headers --- knows only epoch codes.
xcorr therefore never pairs waveforms across a move (correct: the differential geometry is broken by it), and
responses alias epoch $\rightarrow$ base code (a move is not a re-equip). The NS\_100hz mirror is retired (it
was a partial 2021 test build, 8\% coverage, and its per-station swap silently dropped stations in other
years); NS is read native 200\,Hz with the same anti-alias path as GJ.
\end{warnbox}
\begin{center}
\small
\begin{tabular}{@{}lcc l@{}}
...
code from the picker's own probability (\code{config.PHS\_WEIGHT\_SCHEME="probability"}). Design principle
(revised 2026-07): \emph{association is the gatekeeper; the weight is only a prior} --- every associated pick
gets a NONZERO weight, and genuinely bad picks are killed by HYPOINVERSE's own distance + residual re-weighting,
which sees the actual misfit. \code{PHS\_WEIGHT\_BINS} = even quartiles of the emitted range $[0.2, 1.0]$:
$\ge$0.80$\to$0, $\ge$0.60$\to$1, $\ge$0.40$\to$2, else$\to$3 (0 = full weight; \textbf{code 4 is never
assigned}). S picks one code worse (\code{PHS\_WEIGHT\_S\_PENALTY}), \textbf{capped at 3}. Probabilities are
matched to picks by nearest time within \code{PHS\_PROB\_MATCH\_TOL\_S} (50\,ms), alias-aware (BUS/BUS2);
a pick with no match gets the conservative \code{PHS\_WEIGHT\_UNMATCHED\_CODE}=3, not full weight.
Set \code{PHS\_WEIGHT\_SCHEME="phase"} for the legacy fixed P=0/S=1.

\smallskip\noindent The previous scheme ($\ge$0.90/0.70/0.50/0.30, else 4, S+1 uncapped) discarded
\textbf{17.5\%} of associated picks (26\% of S) and matched only 74\% of probabilities, defaulting the rest to
FULL weight; on 2016 it also produced the 92 all-weight-4-P events that segfaulted \code{hyp1.40}. The revision
took 2016 from 11{,}807 located / 7{,}830 QC-pass to \textbf{13{,}123 located (99.9\% of associated) /
9{,}440 QC-pass}, with median RMS unchanged (0.05\,s).
...
\begin{keybox}{One id end-to-end --- the manifest era (2026-07-29)}
Every event is keyed by \code{id = 200000 + event\_idx} (ufpipe's global PyOcto index) in EVERY engine product:
\code{.sum}, \code{.arc}, \code{event.dat}, \code{dt.ct}, \code{dt.cc}, \code{hypoDD.reloc}. The staging step
writes \code{event\_manifest.csv} into the engine run dir and the engine's \code{evmap} assigns cuspids from it
--- never from position over sorted directories, which silently shifted ids whenever stale staging or a
same-second doublet changed the directory count (the 2010/2011 failures). The QC injection is therefore a pure
id \emph{subset} (byte-identical row copy, no renumbering, no time matching), and \code{hypoDD.reloc.dtcc} joins
directly to the PyOcto tables and \code{event\_sac/<event\_idx>/} with no lookup. All hypoDD solves everywhere
use \textbf{LSQR} (\code{ISOLV=2}): the recompiled large-array binary makes SVD pathologically slow at any size.
\end{keybox}
