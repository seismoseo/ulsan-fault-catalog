...
ufpipe orchestrates it. \code{detection\_test/} is now just the 4-picker-comparison pilot plus the shared
per-month detection/association \emph{feeder} that the relocate stage consumes (see \S7). The relocation internals
additionally have a dedicated guide: \code{detection\_test/reloc\_2016\_uf/PIPELINE.md} + \code{study\_guide.pdf}.}
...
\code{ufpipe} is the production end-to-end pipeline. \code{detection\_test} is the pilot experiment (the 2016
4-picker comparison) and also hosts two things ufpipe's \code{relocate} stage reuses: the per-month, multi-network
detection/association \emph{feeder} (\code{detection\_test/lib/}) and the validated relocation orchestrator
(\code{run\_picker\_reloc.py}) that drives the external HypoDD engine.
\begin{center}
\small
...
\midrule
Role & the end-to-end catalog + relocation pipeline & 4-picker-comparison pilot + shared feeder/orchestrator \\
Stages & detection $\rightarrow$ association $\rightarrow$ augment $\rightarrow$ PHS $\rightarrow$ locate $\rightarrow$ \textbf{relocate} & per-month detection/association (\code{lib/}); reloc driver (\code{reloc\_2016\_uf/}) \\
Driver & \code{python -m ufpipe.run\_pipeline} & (called by ufpipe's relocate stage) \\
Ends at & HypoDD dt.cc (\S7) & feeds the relocate stage \\
\bottomrule
\end{tabular}
\end{center}
The \code{relocate} stage feeds on \code{detection\_test/lib}'s per-month, KS/KG/GJ/NS, daily-chunked association
(the dense NS array needs daily chunking), NOT ufpipe's own whole-year association — this is the one place the two
meet. See \S7.
...
Run the relocate stage via ufpipe (\code{-{}-stage-from relocate}); it wraps the validated year-general
orchestrator. Its \textbf{feeder} — the per-month, KS/KG/GJ/NS detection + association — is generated separately in
\code{detection\_test/} (the dense NS array needs daily-chunked association). If a year's feeder is missing, the
relocate stage preflights and prints exactly what to run. The per-step feeder + reloc commands:
...
\code{ufpipe.relocate} (\code{src/ufpipe/relocate.py}) is a \emph{thin wrapper}: it preflights the per-month feeder
inputs then invokes the validated orchestrator \code{detection\_test/reloc\_2016\_uf/run\_picker\_reloc.py}, which
drives the external HypoDD/xcorr engine. Internal stages (after the per-month detection + association feeder,
KS/KG/GJ/NS):
