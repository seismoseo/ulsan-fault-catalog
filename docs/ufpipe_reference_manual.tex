...
detection+association (\code{ufpipe.reloc\_inputs}), so \code{detection\_test/lib/} is DEPRECATED (see
\S7 + \code{detection\_test/lib/DEPRECATED.md}); only the downstream reloc driver + PocketQuake engine remain live.
The relocation internals have a dedicated guide: \code{detection\_test/reloc\_2016\_uf/PIPELINE.md} + \code{study\_guide.pdf}.}
...
4-picker comparison). Its \code{lib/} (per-month detection/association) is DEPRECATED --- ufpipe feeds itself now ---
and only the relocation \emph{orchestrator} \code{reloc\_2016\_uf/run\_picker\_reloc.py} (driven by ufpipe's
relocate stage with \code{-{}-skip-build}) + the external HypoDD engine remain live there.
\begin{center}
\small
...
\midrule
Role & the self-contained end-to-end catalog + relocation pipeline & 4-picker-comparison pilot; hosts the reloc driver + PocketQuake engine (\code{lib/} DEPRECATED) \\
Stages & detection $\rightarrow$ association $\rightarrow$ augment $\rightarrow$ PHS $\rightarrow$ locate $\rightarrow$ \textbf{relocate} & reloc driver (\code{reloc\_2016\_uf/}, \code{-{}-skip-build}) \\
Driver & \code{python -m ufpipe.run\_pipeline} & (called by ufpipe's relocate stage) \\
Ends at & HypoDD dt.cc (\S7) & --- \\
\bottomrule
\end{tabular}
\end{center}
...
relocation internals are documented in \code{reloc\_2016\_uf/PIPELINE.md} + \code{study\_guide.pdf}.
