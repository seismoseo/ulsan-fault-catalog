...
\end{tabular}
\end{center}
ufpipe's own detection + association now cover \textbf{KS/KG/GJ/NS} with daily-chunked association (\S5). The
\code{relocate} stage nonetheless still consumes \code{detection\_test/lib}'s per-month outputs as its feeder
(unchanged and validated); converging relocate onto ufpipe's own association is future work. See \S7.
